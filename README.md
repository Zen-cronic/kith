...
**▶ [Watch the demo](https://youtu.be/1UWK87qQGL0)** · **[Devpost submission](https://devpost.com/software/kith-617bun)** · runs fully offline in deterministic `fake` mode — no AWS account or key needed.

A family's money is never one person's: a household account, a child's education fund, a spouse's insurance to coordinate, a tuition bill due Friday — and most of it falls on one parent. **Kith** runs that shared financial life. It coordinates insurance claims between spouses, tracks a child's education-savings grant room — up to **$7,200 per child in government grants** most families leave on the table — and pays tuition before the late fee, moves household money, and — yes — handles the kids' allowance. Where nearly every other household agent stays safe by never touching money, Kith *safely completes* the work, because code (not the model) decides who may act for whom. It knows exactly **who may decide what for whom**, does the work it is allowed to do and proves it with a **receipt**, and, only when a human must weigh in, surfaces the decision to the responsible adult in the app for a one-tap approval — and emails them the documents it prepared. The number that matters: **0 out-of-scope actions executed across 29 adversarial requests** in the deterministic replay (a live model is measured honestly below).

The agent has a channel-agnostic core with a shared identity and per-channel trust seam (`household/identity.py`, `household/channels/policy.py`), so the same authority engine can reach families over chat, voice, or MCP; Telegram, SMS, and Alexa+ are the roadmap, not shipped today. The web UI is a warm, family-friendly interface with a household-helper mascot (Pip); the honesty chips and the approval surfaces stay deliberately sober so money and consent read clearly (screenshots below).
...
## Surfaces only when a human needs to weigh in

Kith works in the background and makes the safe calls on its own; it interrupts a person only when the ledger says a human must decide. When it does, it reaches the **responsible adult on two channels**:

- **In the app** — the action lands in that adult's queue (the home screen shows "N things waiting on a grown-up," and Pip's face turns to *waiting*), with the exact amount, who must approve, and why. One tap and a PIN release it; a minor can never approve.
- **By email** — the documents Kith prepares for a human to act on (a coordination-of-benefits claim, a rendered form) are sent to the responsible adult as a real Amazon SES email, so the paperwork reaches them even when the app is closed. (The approval decision itself happens in the app with a PIN; an email nudge for approvals is roadmap.)

Nothing that moves money or files a document happens until that human acts, and a prepared official form is *rendered for the adult to review and file* — never submitted for them. This is exactly the flow the Everyday Agents brief asks for: *"work in the background, make the safe calls on their own, and surface only when a human actually needs to weigh in."*

## Architecture

Six Strands agents in a Strands Graph. **The model proposes; code decides; code executes.** Authority is recomputed in code at three points — the `check_authority` tool result the model may only echo, an `AuthorityGuard` that overrides the model after the run, and a tool-call hook (text and voice) that cancels any rail the ledger did not allow.
...
More detail, including the voice, vision, and AgentCore paths, is in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
<!-- guardrails:start -->
**Provider:** `fake` (fake-fixture-model) · **execution mode:** simulated · **run:** 2026-09-14 · **ledger clock:** 2026-09-12T16:00Z (fixed; the seed household is dated for it) · **requests:** 60 (29 out-of-scope, 31 in-scope)
...
**AgentCore Runtime status.** The household container is built and verified as a non-root ARM64 image (digest `sha256:569c1c63…`; serves `/ping`, `/invocations`, and the `/ws` voice path) by `scripts/verify_runtime_image.py`. It is **deployed** to Amazon Bedrock AgentCore Runtime `household_preview` (us-east-1) under a **scoped IAM role and policy — no action wildcards** (the only `Resource:"*"` is `ecr:GetAuthorizationToken`, which AWS does not scope; `bedrock:InvokeModel*` is limited to exactly the three models above), and **verified live**: two household sessions ran on real Bedrock and returned results across five distinct sessions. The sanitized live receipt ([`agentcore-live-2026-09-12.json`](docs/receipts/agentcore-live-2026-09-12.json)) records the passing verifier; the plan and IAM receipt ([`agentcore-2026-09-12.json`](docs/receipts/agentcore-2026-09-12.json)) records the scoped policy and image checks. Region us-east-1. The runbook and the post-approval steps are in [`docs/DEPLOY-AGENTCORE.md`](docs/DEPLOY-AGENTCORE.md).
...
| [`agentcore-live-2026-09-12.json`](docs/receipts/agentcore-live-2026-09-12.json) | Two household sessions on the **deployed** AgentCore runtime returning results on live Bedrock | `python scripts/verify_aws_runtime.py --wheel dist/*.whl` |
| [`agentcore-2026-09-12.json`](docs/receipts/agentcore-2026-09-12.json) | The scoped IAM policy (no wildcards) and ARM64 image verifier checks | `AWS_PROFILE=hackathon-1 python scripts/deploy_runtime_preview.py plan` |
