...
`household/web/**` (P5) and `agentcore/**` (S1) are outside packet P2's mutation scope and still speak the Front Desk
pipeline (five-agent roster, document fixtures, LTB rule ids). Their tests cannot pass against the six-node household
graph until those packets rewrite them, so they are skipped here with the owner named. (P4 replaced the trap-set with
`household/guardrails`, tested in `tests/test_guardrails.py`.)
"""
