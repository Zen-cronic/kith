...
Uses fake models with Memory off and no mounted credentials. Docker Desktop/QEMU
execution is image-contract evidence, not AWS deployment or ARM hardware timing.
"""
...
def main() -> None:
    ...
    receipt = {"imageId": image_id, "requestedTag": args.image, "platform": "linux/arm64", "imageBytes": info["Size"], "sourceCommit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(), "provider": "fake", "cloudDeployment": False}
    try:
        for limit in [20, 3]:
            container = "household-runtime-proof-" + uuid.uuid4().hex[:12]
            docker("run", "--detach", "--name", container, "--platform", "linux/arm64", "--read-only", "--tmpfs", "/tmp:rw,nosuid,size=64m", "--memory", "2g", "--cpus", "2", "--publish", "127.0.0.1::8080", "--env", "MODEL_PROVIDER=fake", "--env", f"MAX_MODEL_CALLS={limit}", "--env", "OTEL_SDK_DISABLED=true", "--env", "AGENTCORE_MEMORY_ID=", image_id)
            ...
            if limit == 3:
                events = invoke(url, {"fixture_id": "school-trip-letter", "language": "es", "max_model_calls": 1000})
                assert events[-1]["code"] == "model_call_limit", events[-1]
                assert events[-1]["model_calls"] == {"limit": 3, "attempted": 3, "remaining": 0, "exhausted": True}
                ...
                continue
            for fixture, outcome in [("ltb-n4", "escalate"), ("school-trip-letter", "proceed"), ("benefits-appointment-letter", "escalate")]:
                events = invoke(url, {"fixture_id": fixture, "language": "es"})
                assert events[0]["event"] == "session_start" and events[-1]["event"] == "result", events[-1]
                result = events[-1]["result"]
                assert result["provider"] == "fake" and result["outcome"] == outcome
                assert result["model_calls"]["attempted"] <= 20 and not result["model_calls"]["exhausted"]
                if fixture == "ltb-n4":
                    assert result["guard"]["rule_id"] == "ON-LTB-N4" and "drafter" not in result["execution_order"]
                    assert "not an eviction order" in result["reading_text"]
                elif fixture == "school-trip-letter":
                    assert [v["decision"] for v in result["verdicts"]] == ["revise", "approve"]
                    assert result["source_form"] and result["drafts"][-1]["preparation_steps"]
                else:
                    assert result["fidelity"]["band"] == "unreliable"
            ...
            events = invoke(url, {"document_text": "Riverdale Branch Library: your requested book is ready for pickup until September 30, 2026.", "title": "Library notice", "language": "es"})
            assert events[-1]["event"] == "result" and events[-1]["result"]["fixture_id"] == "adhoc"
            (out / "raw-document-events.json").write_text(json.dumps(events, indent=2))
            checks.append("raw synthetic document accepted by installed image")
            assert invoke(url, {})[-1]["event"] == "error"
            checks.append("missing document returns an explicit error")
            probe = '''import hashlib,json,os,platform
from pathlib import Path
...
print(json.dumps({'uid':os.getuid(),'architecture':platform.machine(),'python':platform.python_version(),'files':files,'fixtures':len(FixtureStore().list()),'packages':{d.metadata['Name'].lower().replace('_','-'):d.version for d in distributions()}}))
'''
            ...
            for package in ['household','strands-agents','strands-agents-tools','bedrock-agentcore','fastapi','pydantic','uvicorn']:
            ...
            checks.append("installed main dependency versions match lock; required packages and46 fixtures present")
