...
- **AgentCore** — the same graph runs on Amazon Bedrock AgentCore Runtime via `agentcore/app.py` (streaming `/invocations` text path and `/ws` voice path). The container is built and verified as a non-root ARM64 image; the deployment is fully planned with a scoped IAM role (no action wildcards) and paused at a human approval gate. The runbook, the post-approval steps, and the sanitized plan receipt are in [`DEPLOY-AGENTCORE.md`](DEPLOY-AGENTCORE.md) and [`receipts/agentcore-2026-09-12.json`](receipts/agentcore-2026-09-12.json).
